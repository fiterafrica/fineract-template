...
UPDATE stretchy_report SET report_sql = " SELECT
                                              ...
                                              IFNULL(penaltiesTbl.Pen_charged, 0) AS 'Total Late Fees',
                                              IFNULL(l.total_overpaid_derived, 0) AS 'Excess Amount Paid',
                                              IFNULL(waiverTblInterest.int_waived, 0) AS 'Interest Waived',
                                             IFNULL(waiverTblCharges.fee_waived, 0) AS 'Fee Waived',
                                             IFNULL(penaltiesTbl.pen_waived, 0) AS 'Pen Waived',
                                             l.fee_charges_charged_derived as 'Insurance fee Charged',
                                              CASE
                                                  WHEN l.loan_status_id IN (300, 600, 601, 602, 700)
                                                      THEN ((l.principal_disbursed_derived + l.interest_charged_derived + l.fee_charges_charged_derived + ifnull(penaltiesTbl.Pen_charged, 0))
                                                          - l.fee_charges_repaid_derived
                                                          - IFNULL(waiverTblCharges.fee_waived, 0)
                                                          - IFNULL(paymentTbl.princ_paid, 0)
                                                          - IFNULL(paymentTbl.int_paid, 0)
                                                          - IFNULL(waiverTblInterest.int_waived, 0)
                                                          - IFNULL(penaltiesTbl.pen_waived, 0)
                                                          - IFNULL(paymentTbl.pen_paid, 0)
                                                          - IFNULL(writtenOffPayment.total_writtenOff, 0))
                                                  ELSE 0
                                              END AS 'Current Balance',
                                              CASE
                                                  WHEN l.loan_status_id IN (300, 600, 601, 602, 700)
                                                      THEN (l.principal_disbursed_derived - IFNULL(paymentTbl.princ_paid, 0) - IFNULL(writtenOffPayment.principal_writtenOff, 0))
                                                  ELSE 0
                                              END AS 'Principal Balance',
                                              CASE
                                                  WHEN l.loan_status_id IN (300, 600, 601, 602, 700)
                                                      THEN (l.interest_charged_derived - IFNULL(paymentTbl.int_paid, 0) - IFNULL(waiverTblInterest.int_waived, 0) - IFNULL(writtenOffPayment.int_writtenOff, 0))
                                                  ELSE 0
                                              END AS 'Interest Balance',
                                              0 AS 'Fees Balance',
                                              (
                                                  CASE
                                                      WHEN (expected_princ - IFNULL(paymentTbl.princ_paid, 0)) > 0 THEN (expected_princ - IFNULL(paymentTbl.princ_paid, 0) - IFNULL(writtenOffPayment.principal_writtenOff, 0))
                                                      ELSE 0
                                                  END
                                                  +
                                                  CASE
                                                      WHEN (expected_int - IFNULL(waiverTblInterest.int_waived, 0) - IFNULL(paymentTbl.int_paid, 0)) > 0 THEN (expected_int - IFNULL(waiverTblInterest.int_waived, 0) - IFNULL(paymentTbl.int_paid, 0) - IFNULL(writtenOffPayment.int_writtenOff, 0))
                                                      ELSE 0
                                                  END
                                                  +
                                                  CASE
                                                      WHEN (expected_fees - IFNULL(paymentTbl.fee_paid, 0) - IFNULL(waiverTblCharges.fee_waived, 0)) > 0 THEN (expected_fees - IFNULL(paymentTbl.fee_paid, 0) - IFNULL(waiverTblCharges.fee_waived, 0)  - IFNULL(writtenOffPayment.fee_writtenOff, 0))
                                                      ELSE 0
                                                  END
                                              ) AS 'Amount Past Due',
                                              CASE
                                          ...
                                          FROM
                                              ...
                                              LEFT JOIN (
                                                  ...
                                                  FROM m_loan_transaction t
                                                  WHERE t.transaction_type_enum IN (1, 2, 28)
                                                      AND t.is_reversed = 0
                                                      AND (DATE(t.transaction_date) BETWEEN DATE('${startDate}') AND DATE('${endDate}'))
                                                  GROUP BY t.loan_id
                                              ...
                                              LEFT JOIN (
                                                  ...
                                                  WHERE (DATE(t.due_for_collection_as_of_date) BETWEEN DATE('${startDate}') AND DATE('${endDate}')) and t.is_active = 1 and t.is_penalty
                                                  GROUP BY t.loan_id
                                              ) penaltiesTbl ON penaltiesTbl.loan_id = l.id
                                              LEFT JOIN (
                                                  SELECT
                                                      t.loan_id,
                                                      SUM(IFNULL(t.amount, 0)) AS int_waived
                                                  FROM m_loan_transaction t
                                              ...
                                              LEFT JOIN (
                                                  ...
                                                  FROM m_loan_transaction t
                                                  WHERE t.transaction_type_enum = 9
                                                      AND t.is_reversed = 0
                                                      AND (DATE(t.transaction_date) BETWEEN DATE('${startDate}') AND DATE('${endDate}'))
                                                  GROUP BY t.loan_id
                                              ) waiverTblCharges ON waiverTblCharges.loan_id = l.id
                                              LEFT JOIN (
                                                  SELECT
                                                      t.loan_id,
                                                      SUM(IFNULL(t.principal_portion_derived, 0)) AS principal_writtenOff,
                                                      SUM(IFNULL(t.interest_portion_derived, 0)) AS int_writtenOff,
                                                      SUM(IFNULL(t.fee_charges_portion_derived, 0)) AS fee_writtenOff,
                                                      SUM(IFNULL(t.penalty_charges_portion_derived, 0)) AS pen_writtenOff,
                                                      SUM(IFNULL(t.amount, 0)) AS total_writtenOff
                                                  FROM m_loan_transaction t
                                                  WHERE t.transaction_type_enum = 6
                                                      AND t.is_reversed = 0
                                                      AND (DATE(t.transaction_date) BETWEEN DATE('${startDate}') AND DATE('${endDate}'))
                                                  GROUP BY t.loan_id
                                              ) writtenOffPayment ON writtenOffPayment .loan_id = l.id
                                              ...
                                              LEFT JOIN (
                                                  SELECT
                                                      ...
                                                      lrs.loan_id
                                                  FROM m_loan_repayment_schedule lrs
                                                  WHERE (DATE(lrs.duedate) BETWEEN DATE('${startDate}') AND DATE(DATE_SUB('${endDate}', INTERVAL 1 DAY)))
