...
public class CamelConfig {
    ...
    public RedeliveryPolicy redeliveryPolicy() {
        ...
        policy.redeliveryDelay(500L);

        return policy;
    }

    @Bean
    public DeadLetterChannelBuilder deadLetterChannelBuilder(RedeliveryPolicy policy) {
        DeadLetterChannelBuilder builder = new DeadLetterChannelBuilder();
        builder.setDeadLetterUri(CamelEndpoints.SEDA_FINERACT_DEADLETTER);
