...
SELECT
 ...
 IFNULL(waiverTblCharges.fee_waived, 0) AS 'Fee Waived',
    ...
    CASE
        WHEN l.loan_status_id IN (300, 600, 601, 602, 700)
            THEN (l.interest_charged_derived - IFNULL(paymentTbl.int_paid, 0) - IFNULL(waiverTblInterest.int_waived, 0))
        ELSE 0
    ...
    (
        ...
        +
        CASE
            WHEN (expected_int - IFNULL(waiverTblInterest.int_waived, 0) - IFNULL(paymentTbl.int_paid, 0)) > 0 THEN (expected_int - IFNULL(waiverTblInterest.int_waived, 0) - IFNULL(paymentTbl.int_paid, 0))
            ELSE 0
        END
        +
        CASE
            WHEN (expected_fees - IFNULL(paymentTbl.fee_paid, 0) - IFNULL(waiverTblCharges.fee_waived, 0)) > 0 THEN (expected_fees - IFNULL(paymentTbl.fee_paid, 0) - IFNULL(waiverTblCharges.fee_waived, 0))
            ELSE 0
        END
    ) AS 'Amount Past Due',
    CASE
